    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-blue-700 pb-32">
      ...
      {activeTab === 'inicio' && (
        <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
          {/* 1. Estado de Cuenta */}
          <div className={`rounded-3xl p-8 shadow-2xl transition-all ${isSolvent ? 'bg-gradient-to-br from-green-500 to-green-600' : 'bg-gradient-to-br from-blue-600 to-blue-700'}`}>
        ...
        <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
          <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-4 border border-white/20">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => changeMonth(1)} className="p-2 rounded-lg hover:bg-white/10 transition-all"><ChevronRight className="w-6 h-6 text-white" /></button>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2 px-2 overflow-x-auto">
              {['todos', 'approved', 'pending', 'rejected'].map((filter) => (
                <button 
                  key={filter}
                  onClick={() => setPaymentFilter(filter)}
                  className={`px-4 py-2 rounded-full text-sm whitespace-nowrap transition-all ${
                    paymentFilter === filter ? 'bg-blue-600 text-white' : 'bg-white/10 text-white/70 hover:bg-white/20'
                  }`}
                >
              ...
              {['inicio', 'finanzas', 'comunidad'].map((tab) => (
                <button 
                  ...
                  className={`flex flex-col items-center gap-2 py-3 px-4 rounded-2xl transition-all ${
                    activeTab === tab ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30 scale-105' : 'hover:bg-gray-100 text-gray-600'
                  }`}
                >
