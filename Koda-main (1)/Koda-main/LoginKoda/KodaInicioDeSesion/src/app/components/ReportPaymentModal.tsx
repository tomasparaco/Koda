    ...
    if (isOpen && userData?.id_propietario) {
      const fetchRecibos = async () => {
        ...
        if (exito && data) {
          ...
          if (data.length > 0) {
            setFormData(prev => ({ ...prev, id_recibo: data[0].id_recibo.toString() }));
          }
        }
      ...
      <div className="relative bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-fade-in">
        ...
        {isSuccess ? (
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                {isLoadingRecibos ? (
                  ...
                  <div className="w-full px-4 py-2.5 bg-yellow-50 border border-yellow-200 rounded-xl text-yellow-700">
                  ...
                  >
                    {pendingRecibos && pendingRecibos.map((recibo) => (
                      <option key={recibo.id_recibo} value={recibo.id_recibo}>
                        Recibo N° {recibo.id_recibo} - Bs. {Number(recibo.monto_total).toFixed(2)} (Periodo: {new Date(recibo.periodo).toLocaleDateString('es-ES', { month: 'long', year: 'numeric' })})
                      </option>
                    ))}
                  </select>
                )}
            ...
            <button
              type="submit"
              disabled={isSubmitting || !pendingRecibos || pendingRecibos.length === 0}
              className="w-full mt-2 bg-blue-600 text-white font-medium py-3 rounded-xl hover:bg-blue-700 transition-all flex items-center justify-center disabled:opacity-70 disabled:bg-gray-400"
            >
              {isSubmitting ? (
                <>
