...
// componente auxiliar que dispara la animación cuando entra en el viewport (scroll)
...
export const container: Variants = {
  ...
  visible: {
    transition: {
      staggerChildren: 0.12,    // tiempo entre cada hijo
      when: 'beforeChildren',   // primero el contenedor, luego los hijos
    },
  },
};
...
export default function App() {
  ...
  // A key that changes whenever we switch to a different screen/view; used to trigger
  // a CSS animation on the container each time the user navigates.
  const viewKey = useMemo(() => {
    if (propiedades.length > 0) {
      if (!selectedPropiedad) return 'selector';
      return `${selectedPropiedad.rol}-${selectedPropiedad.id_propietario}`;
    }
    if (isUpdateFlow) return 'update';
    if (isResetMode) return 'reset';
    return 'login';
  }, [propiedades.length, selectedPropiedad, isUpdateFlow, isResetMode]);
  ...
  // Instead of returning directly from each branch we build the content and
  // render it inside a wrapper with a key. Changing the key forces React to
  // remount the container which in turn retriggers the CSS animation.
      ...
      if (propiedadActiva.rol === 'admin') {
        contenido = (
          <DashboardLayout
            ...
            onLogout={handleLogout}
            onBackToSelector={() => setSelectedPropiedad(null)}
          />
        );
      } else {
            ...
            <div className="bg-white rounded-3xl p-6 shadow-xl">
              <form onSubmit={handleSubmit}>
                {isUpdateFlow && (
                  <>
                    <div className="mb-4 relative">
                      ...
                      <input type={showNewPassword ? "text" : "password"} placeholder="Nueva contraseña" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required className="w-full pl-12 pr-12 py-3.5 bg-gray-100 rounded-xl focus:bg-white outline-none focus:border-[#4A7FDB] border-2 border-transparent transition-all" />
                      <button type="button" onClick={() => setShowNewPassword(!showNewPassword)} className="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-[#4A7FDB] transition-colors">
                        {showNewPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                      </button>
                    </div>
                    <div className="mb-4 relative">
                      <div className="absolute left-4 top-1/2 transform -translate-y-1/2"><Lock className="w-5 h-5 text-[#4A7FDB]" /></div>
                      <input type={showConfirmPassword ? "text" : "password"} placeholder="Confirmar contraseña" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required className="w-full pl-12 pr-12 py-3.5 bg-gray-100 rounded-xl focus:bg-white outline-none focus:border-[#4A7FDB] border-2 border-transparent transition-all" />
                      <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-[#4A7FDB] transition-colors">
                ...
                {!isUpdateFlow && !isResetMode && (
                  <div className="mb-4 relative">
                    <div className="absolute left-4 top-1/2 transform -translate-y-1/2"><Lock className="w-5 h-5 text-[#4A7FDB]" /></div>
                    <input type={showPassword ? "text" : "password"} placeholder="Contraseña" value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full pl-12 pr-12 py-3.5 bg-gray-100 rounded-xl focus:bg-white outline-none focus:border-[#4A7FDB] border-2 border-transparent transition-all" />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-[#4A7FDB] transition-colors">
  ...
  // envolvemos el contenido en un motion.div para que cada vez que cambie
  // `viewKey` (navegación entre pantallas) se remonte el contenedor y se
  // reproduzca el slide-up + fade-in con stagger. Anteriormente usábamos la
  // clase `animate-rise` de Tailwind, ahora aprovechamos los `variants` ya
  // declarados arriba.
  return (
    <motion.div
      key={viewKey}
