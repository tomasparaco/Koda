...
export default function ReportPaymentModal({ isOpen, onClose, userData }: ReportPaymentModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Notificar al admin sobre el nuevo pago reportado
      await NotificacionService.crear({
        codigo_edificio: userData.codigo_edificio,
        tipo: 'pago',
        titulo: 'Nuevo Pago Reportado',
        mensaje: `Apto ${userData.apartamento} reportó un pago de $${parseFloat(formData.monto).toFixed(2)} (${formData.metodo}).`,
        destinatario: 'admin',
      });
