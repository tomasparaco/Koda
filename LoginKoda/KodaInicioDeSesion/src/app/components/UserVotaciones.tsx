...
export default function UserVotaciones({ userData }: Props) {
  ...
  useEffect(() => {
    if (userData.codigo_edificio) cargarDatos();
    
    // Simular tiempo real con un intervalo (O usar Supabase Realtime si está activo)
  ...
  const cargarDatos = async () => {
    ...
    // Verificación de fecha para visualización correcta
  ...
  const handleVotar = async (encuesta_id: string) => {
    ...
    if (res.exito) {
      alert("¡Tu voto ha sido registrado con éxito de forma anónima!");
      cargarDatos();
    } else {
      alert(res.mensaje);
    }
  ...
  if (loading) return <div className="text-center text-white/50 py-10 animate-pulse">Cargando asambleas...</div>;

  return (
    <div className="animate-fade-in space-y-6 pb-20 max-w-4xl mx-auto px-4 py-6">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-white flex items-center gap-2">
          <Vote className="w-6 h-6" /> Asambleas y Votaciones
        </h2>
        <p className="text-white/60">Participa en las decisiones de tu comunidad (Apt. {userData.apartamento})</p>
      </div>
      ...
      {encuestas.length === 0 ? (
        ...
        <div className="grid gap-6">
          {encuestas.map((enc) => {
            // Verificamos si EL APARTAMENTO del usuario ya votó
            ...
            return (
              <div key={enc.id} className="bg-white/10 border border-white/20 backdrop-blur-lg rounded-2xl p-6 shadow-xl relative overflow-hidden">
                {/* Indicador de estado */}
                <div className={`absolute top-0 left-0 w-1 h-full ${enc.activa ? 'bg-green-500' : 'bg-red-500'}`} />
                ...
                <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-4">
                  ...
                  {yaVoto && (
                    <div className="flex items-center gap-1 text-green-400 bg-green-400/10 px-3 py-1.5 rounded-full text-sm font-medium border border-green-400/20">
                      <CheckCircle className="w-4 h-4" /> Tu voto fue registrado
                    </div>
                  )}
                </div>
                ...
                {enc.activa && !yaVoto ? (
                  // AÚN NO HA VOTADO Y ESTÁ ACTIVA -> Mostrar Radio Buttons
                    ...
                    >
                      {enc.opciones.map((op: string) => (
                        <div key={op} className="flex items-center space-x-3 bg-white/5 p-3 rounded-lg border border-white/10 hover:bg-white/10 transition-colors cursor-pointer">
                          <RadioGroupItem value={op} id={`${enc.id}-${op}`} className="border-white/50 text-blue-400" />
                          <Label htmlFor={`${enc.id}-${op}`} className="text-white cursor-pointer w-full text-base">{op}</Label>
                        </div>
                      ))}
                    </RadioGroup>
                    <Button 
                      ...
                      className="w-full mt-6 bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-900/20"
                    >
                      {enviando ? 'Registrando...' : 'Confirmar Voto'}
                    </Button>
                  </div>
                ) : (
                  // YA VOTÓ O ESTÁ CERRADA -> Mostrar Resultados
                  <div className="space-y-4 mt-2">
                    <p className="text-white/70 text-sm mb-2 flex items-center gap-2 font-medium">
                      <BarChart3 className="w-4 h-4"/> Resultados {enc.activa && "(En vivo)"} - {totalVotos} votos totales
                    </p>
                        ...
                        return (
                          <div key={op} className="space-y-1">
                            ...
                            <div className="w-full bg-black/40 rounded-full h-3 overflow-hidden border border-white/5">
                              <div className="bg-gradient-to-r from-blue-600 to-blue-400 h-full rounded-full transition-all duration-1000 ease-out" style={{ width: `${porcentaje}%` }} />
                            </div>
                          </div>
                        );
                      })}
