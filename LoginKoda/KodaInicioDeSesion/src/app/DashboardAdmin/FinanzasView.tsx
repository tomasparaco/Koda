...
export function FinanzasView({ propiedad }: FinanzasViewProps) {
  ...
  const [selectedPaymentForApprove, setSelectedPaymentForApprove] = useState<{ id_pago: number, id_recibo: number, id_propietario: number, monto: number } | null>(null);
  ...
  const handleAprobarClick = (id_pago: number, id_recibo: number, id_propietario: number, monto: number) => {
    setSelectedPaymentForApprove({ id_pago, id_recibo, id_propietario, monto });
          ...
          {pendingPayments.length === 0 ? (
            ...
            <div key={payment.id_pago} className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5">
              ...
              <div className="flex gap-3 mt-4">
                <button
                  onClick={() => handleAprobarClick(payment.id_pago, payment.id_recibo, payment.recibos.id_propietario, payment.monto)}
